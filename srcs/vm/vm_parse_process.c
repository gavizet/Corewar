...
/*   Updated: 2018/03/31 14:08:20 by gavizet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_parse_process_get_prog(t_process **process, int fd)
{
	...
	{
		free(*process);
		ft_error("INVALID FORMAT (ERROR 0x03 PROG_SIZE DOES NOT MATCH)");
	}
}
...
static void		ft_parse_process_prog_size(t_process **process, int fd)
{
	...
	{
		free((*process));
		ft_error("INVALID FORMAT (ERROR 0x03 PROG_SIZE)");
	}
...
static void		ft_parse_process_part_two(t_process **process, int fd)
{
	...
	{
		free(*process);
		ft_error("INVALID FORMAT (ERROR 0x02 NAME)");
	}
	lseek(fd, 4, SEEK_CUR);
	...
	{
		free(*process);
		ft_error("INVALID FORMAT (ERROR 0x04 COMMENT)");
	}
	lseek(fd, 4, SEEK_CUR);
}
...
static void		ft_parse_process_magic_number(t_process **process, int fd)
{
	...
	{
		free((*process));
		ft_error("INVALID FORMAT (ERROR 0x01 MAGIC NUMBER)");
	}
									...
									t_vm *vm, int i)
{
	...
	if ((process = (t_process *)malloc(sizeof(t_process))))
	{
		if ((fd = open(cor_file_name, O_RDONLY)) != -1)
		{
			...
			while (++i < 16)
				process->reg[i] = 0;
			ft_parse_process_magic_number(&process, fd);
